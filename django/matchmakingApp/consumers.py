...
class GameLoopConsumer:
	def __init__(self, game_id, channel_layer, p1, p2):
		...
		self.game_state = {
			...
			'score': [0, 0],
			'speed': 200 * self.frameTime,
			'paddle1': [1, self.game_const.board.y / 2 - self.game_const.paddle.height / 2],
			'paddle2': [self.game_const.board.x - self.game_const.paddle.width, self.game_const.board.y / 2 - self.game_const.paddle.height / 2],
		...
		asyncio.create_task(self.send_game_data())
	...
	async def send_game_data(self):
		await self.channel_layer.group_send(
			...
			{
				'type': 'gameConst',
				'gameConst': asdict(self.game_const)
			}
		)
	...
	def check_collision(self):
		...
		#check for paddle colision left/right, i.e. also for score
			...
			if not self.colidePaddle("paddle1" if self.game_state["vector"][0] <= 0 else "paddle2"):
				if self.OutOfBound():
	...
	def colidePaddle(self, paddle):
		...
		if paddle == "paddle1":
			self.game_state["vector"][0] = abs(self.game_state["vector"][0])
		else:
			self.game_state["vector"][0] = -abs(self.game_state["vector"][0])
		return True
	...
	def increaseSpeed(self, paddle):
		...
		if (paddle == "paddle1" and (self.game_state[self.players[0]]["ArrowDown"] or self.game_state[self.players[0]]["ArrowUp"])) \
			...
			if self.game_state["speed"] < self.game_const.initSpeed * 3: 
	...
	def checkEndGame(self):
		if abs(self.game_state["score"][0] - self.game_state["score"][1]) > 1:
...
class matchmakingConsumer(AsyncWebsocketConsumer):
	...
	async def message(self, event):
		await self.send(text_data=json.dumps({'gameState': event}))

	async def gameConst(self, event):
		await self.send(text_data=json.dumps({'gameConst': event}))
